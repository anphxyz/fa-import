$.fn.doPagination = function (total_row, url, limit, callback) {
    ...
    $container.addClass('page-first')    //btnFirst->btnPrev-> spnFst
            .append($('<a>', {lang: target + '&pnOFFSET=1', class: 'func go-page-first inuse', num: '1'})
    ...
    for (var i = 1; i <= pageCnt; i++) //show top 3 tag
        $container.append($('<a>', {lang: target + '&pnOFFSET=' + ((i - 1) * limit + 1), class: 'gopgno' + (i === 1 ? ' active' : '') + (i <= 5 ? ' inuse' : ' hidden'), html: i, num: i + ''}));
    //hide when not top 3 tag
    $container// spnLst->btnNext->btnLast
            .append($('<span>', {class: 'spn last' + (pageCnt < 4 ? ' hidden' : ''), html: '...'}))
            .append($('<a>', {lang: target + '&pnOFFSET=' + (limit + 1), class: 'func next inuse', num: '2'})
                    .append($('<span>', {class: 'fa fa-angle-right fa-lg'})))
            .append($('<a>', {lang: target + '&pnOFFSET=' + ((pageCnt - 1) * limit + 1), class: 'func go-page-last inuse', num: pageCnt + ''})
                    .append($('<span>', {class: 'fa fa-angle-double-right fa-lg'})));
    ...
    $container.on('click', 'a', function () {
        var $self = $(this),
                pgNo = +$self.attr('num'),
                $containerCls = 'qb-pagination image page-' + (pgNo === 1 ? 'first' : (pgNo === pageCnt ? 'last' : 'no')),
        ...
        $container.find('a.prev').attr('num', (pgNo - 1))
                .attr('lang', target + '&pnOFFSET=' + prevOffset);
        $container.find('a.next').attr('num', (pgNo + 1))
                .attr('lang', target + '&pnOFFSET=' + nextOffset);
